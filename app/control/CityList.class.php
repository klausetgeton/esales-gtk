...
class CityList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TForm('form_search_City');
        ...
        $this->form->add($table);
        $table->addRowSet(new TLabel(_t('Cities')), '')->class='tformtitle';
        
        // create the form fields
        $filter = new TEntry('name');
        $filter->setSize(400);
        $filter->setValue(TSession::getValue('City_name'));
        ...
        $new_button->setAction(new TAction(array('CityForm', 'onEdit')), _t('New'));
        ...
        $this->datagrid->addQuickColumn(_t('Name'), 'name', 'left', 400, new TAction(array($this, 'onReload')), array('order', 'name'));

        // add the actions to the datagrid
        $this->datagrid->addQuickAction(_t('Edit'), new TDataGridAction(array('CityForm', 'onEdit')), 'id', 'ico_edit.png');
