 ...
 */
class VendaTeste extends TPage
{
    private $form; // form
...
/*        // security check
        ...
        // creates the form
        $this->form = new TForm('form_Venda');
        ...
        $notebook->appendPage(_t('Venda'), $table1);
        $notebook->appendPage(_t('Itens'), $table2);
/*        $notebook->appendPage(_t('Subjects'), $table3);
        ...
        $action->setParameter('parent',        'form_Venda');
        $action->setParameter('model',         'Venda');
        ...
        $row=$table1->addRow();
        $row->addCell(new TLabel(_t('Código')));
        $cell=$row->addCell($id);
        $cell->colspan=3;

        // add a row for the field title
        $row=$table1->addRow();
        $row->addCell(new TLabel(_t('Data')));
        $cell=$row->addCell($data_venda);
        $cell->colspan=3;
        ...
        $row=$table1->addRow();
        $row->addCell(new TLabel('Publisher' . ': '));
        ...
        $this->form->setFields(array($id,$data_venda,$vendedor_id,$vendedor_nome, $produtos_teste, $save_button));
